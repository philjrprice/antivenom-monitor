...
if p_toxic > safe_conf_req:
    st.error(f"🛑 **GOVERNING RULE: SAFETY STOP.** SAE Risk ({p_toxic:.1%}) ≥ {safe_conf_req:.0%} threshold.")
elif is_look_point:
    if bpp < bpp_futility_limit: 
        ...
        st.info("🛡️ **GOVERNING RULE: CONTINUE.** Data is currently indeterminate.")
elif total_n < min_interim:
    st.info(f"⏳ **STATUS: LEAD-IN.** Lead-in enrollment; first check at N={min_interim}.")
else:
    next_check = total_n + (check_cohort - (total_n - min_interim) % check_cohort)
    st.info(f"🧬 **STATUS: MONITORING.** Between cohorts. Next check at N={next_check}.")
...
if include_heatmap:
    ...
    score = E - (2 * S)
    fig_heat = px.imshow(score, x=eff_grid, y=saf_grid, labels=dict(x="Efficacy", y="SAE Rate"), color_continuous_scale="RdYlGn")
...
for i, (name, ap, bp) in enumerate(priors_list):
    p_t_s = 1 - beta.cdf(target_eff, ap + successes, bp + (total_n - successes))
    with cols[i]:
        st.info(f"**{name}**")
...
if st.button("📥 Export Audit-Ready Snapshot"):
    report_data = {
        "Metric": ["Timestamp", "N", "Successes", "SAEs", "Post Mean Eff", "Prob > Target", "PPoS", "Bayes Factor"],
        "Value": [datetime.now().isoformat(), total_n, successes, saes, f"{eff_mean:.2%}", f"{p_target:.2%}", f"{bpp:.2%}", f"{evidence_shift:.2f}"]
    }
