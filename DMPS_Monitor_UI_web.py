...
with st.expander("📊 Full Statistical Breakdown", expanded=True):
    ...
    with c3:
        ...
        st.write(f"ESS (Effective Sample N): **{a_eff + b_eff:.1f}**", help="Total information content in the model, calculated as the Current Sample Size (N) plus the sum of your Prior Weights (Alpha + Beta).")
...
for i, (name, ap, bp) in enumerate(priors_list):
    ...
    with cols[i]:
        ...
        if "Neutral" in name: st.write(f"Bayes Factor (BF₁₀): **{evidence_shift:.2f}x**", help="The ratio of the likelihood of the alternative hypothesis (Target Efficacy) to the null hypothesis. A value >1 indicates the data supports the treatment effect.")
