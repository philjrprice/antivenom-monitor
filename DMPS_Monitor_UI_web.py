...
# RESTORED: Probability checks against all three thresholds
...
with st.expander("📊 Full Statistical Breakdown", expanded=True):
    ...
    with c1:
        ...
        # RESTORED: Stats against all three levels
...
for i, (name, ap, bp) in enumerate(priors_list):
    ...
    with cols[i]:
        st.info(f"**{name}**")
        st.write(f"P(>Target): {p_t_s:.1%}")

# RESTORED: Sensitivity interpretation logic
